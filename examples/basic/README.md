...
| `GET /token/voice`       | Minting a Voice access token from an API-key-authenticated named client                                             |
| `GET /token/video`       | Same, scoped to a Video room                                                                                        |
...
To exercise the success path, compute the signature the way Twilio does.
Append the URL, then every POST parameter, in alphabetical order, signed with
your auth token:
